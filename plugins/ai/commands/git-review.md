---
description: Run a quick git diff review of uncommitted changes or a branch
...
- Run:
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/ai-companion.mjs" git-review "$ARGUMENTS"
```
...
```typescript
Bash({
  command: `node "${CLAUDE_PLUGIN_ROOT}/scripts/ai-companion.mjs" git-review "$ARGUMENTS"`,
  description: "AI git review",
  run_in_background: true
})
```
- Do not call `BashOutput` or wait for completion in this turn.
- After launching the command, tell the user: "Git review started in the background. Check `/ai:status` for progress."
