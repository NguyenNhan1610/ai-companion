...
The provider (Codex, Claude) is selected via `--model provider:model`. Claude tiers: `--model claude:fast` (Haiku), `--model claude:code` (Sonnet, default), `--model claude:max` (Opus with 1M context).
