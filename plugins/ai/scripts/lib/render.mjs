...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    const actions = [`/ai:status ${job.id}`];
    if (job.status === "queued" || job.status === "running") {
      actions.push(`/ai:cancel ${job.id}`);
    }
    lines.push(
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: /ai:cancel ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: /ai:result ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Review changes: /ai:review --wait");
    lines.push("  Stricter review: /ai:adversarial-review --wait");
  }
...
export function renderTaskResult(parsedResult, meta) {
  ...
  const message = String(parsedResult?.failureMessage ?? "").trim() || "AI backend did not return a final message.";
  return `${message}\n`;
}
...
export function renderStatusReport(report) {
  ...
  if (report.needsReview) {
    ...
    lines.push("Ending the session will trigger a fresh adversarial review and block if it finds issues.");
  }
...
export function renderStoredJobResult(job, storedJob) {
  ...
  if (threadId) {
    lines.push(`Session ID: ${threadId}`);
    lines.push(`Resume in Codex: ${resumeCommand}`);
  }
...
export function renderCancelReport(job) {
  ...
  lines.push("- Check `/ai:status` for the updated queue.");
