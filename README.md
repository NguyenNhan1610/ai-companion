...
| Native statusline | None | **`/ai:setup --install-statusline`** — per-request token metrics (4 cache tiers) + session cost/duration/speed via native stdin contract |
...
| `/ai:setup` | Backend readiness, init project, install rules/mermaid/statusline, review gate |
...
/ai:setup --install-statusline            # Install native token/session metrics statusline
```
...
## Native StatusLine — Per-Request Token & Session Metrics

```bash
/ai:setup --install-statusline            # One-time setup
```

Replaces the default bash statusline with a Node.js handler that reads Claude Code's native stdin JSON contract — the same authoritative data that `/context` shows. No estimation, no transcript parsing.

### What You See

**Normal context (<85%):**
```
[Opus 4.6] ████░░░░░░ 45% │ in: 61k out: 7k │ $0.42
speed: 42.1 tok/s │ api: 63% │ 5m │ +120 -30
```

**High context (≥85%) — per-bucket breakdown:**
```
[Opus 4.6] █████████░ 88% │ in: 150k cw: 20k cr: 22k │ $1.52
```

### Token Metrics (Line 1)

| Field | Source | Description |
|-------|--------|-------------|
| `input_tokens` | `context_window.current_usage` | Uncached input tokens sent |
| `output_tokens` | `context_window.current_usage` | Tokens generated by model |
| `cache_creation_input_tokens` | `context_window.current_usage` | Tokens written to prompt cache |
| `cache_read_input_tokens` | `context_window.current_usage` | Tokens read from prompt cache |
| `used_percentage` | `context_window` | Native context % (v2.1.6+), accounts for autocompact buffer |

All values are cumulative session totals, updated after each API response. At ≥85% context, the compact `in/out` view switches to a per-bucket breakdown (`in/cw/cr`) so you can see which cache tier is consuming context.

### Session Metrics (Line 2)

| Field | Source | Description |
|-------|--------|-------------|
| Output speed | Delta between invocations | `tok/s` from consecutive `output_tokens` diffs |
| API ratio | `cost.total_api_duration_ms / total_duration_ms` | % of session spent waiting for API vs tool execution |
| Duration | `cost.total_duration_ms` | Wall-clock session duration |
| Lines changed | `cost.total_lines_added/removed` | Net code changes across all file edits |
| Cost | `cost.total_cost_usd` | Native billing-accurate session cost |

### Why Native Over Estimation

- **Autocompact-aware** — reflects post-compaction state, not stale transcript sums
- **1M-context accurate** — 1% error at 1M tokens = 10K tokens; native data eliminates drift
- **4 cache tiers** — separate `input`, `output`, `cache_write`, `cache_read` with correct cost multipliers
- **Parity with `/context`** — identical numbers, no user confusion
- **41ms runtime** — single JSON parse (~1KB), well within the 300ms statusLine budget

### Fallback Behavior

| Condition | Behavior |
|-----------|----------|
| `used_percentage` absent (pre-v2.1.6) | Manual: `tokens / window_size × 100` |
| `current_usage` absent | Shows `ctx: --` |
| `cost.total_cost_usd` absent | Hidden (no estimation fallback) |
| Empty stdin / timeout | `[AI Companion] Initializing...` |
...
This fork adds 25 commands (vs 7), 5 focused review commands, aspect-based reviews (28 templates, 3 languages, 5 techstacks), multi-agent council, hypothesis debugging, full document flow (ADR/FDR/TP/IMPL/TODO) with acceptance criteria hierarchy, pairwise stage validation with auto-validation hooks, `/ai:plan-feature` full pipeline, knowledge extraction with auto-suggestion, coding rules injected into fork agents and external backends, native statusLine with per-request token metrics (4 cache tiers) and session cost/speed/duration, cascade tracking with timestamps, batch lint on Stop, Mermaid rendering, and auto-installable coding rules. The original only supports diff-based reviews.
